...
interface ParkPosterEditDialogProps {
  open: boolean;
...
export function ParkPosterEditDialog({ open, onOpenChange, park, onSuccess }: ParkPosterEditDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [photoUrl, setPhotoUrl] = useState(park.poster || "");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/parks/${park.slug}/photo`, {
        method: "POST",
        body: formData,
      });
      ...
      toast.success("Park poster updated successfully");
      onSuccess();
      onOpenChange(false);
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to update poster URL");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-media-surface text-media-primary border-media-outline-variant/20 font-lexend">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold tracking-tight">Edit Park Poster</DialogTitle>
          <DialogDescription className="text-media-on-surface-variant">
            Update the main image for &quot;{park.title}&quot;.
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="url" className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input 
                  id="photo-url" 
                  placeholder="https://example.com/park.jpg" 
                  value={photoUrl}
