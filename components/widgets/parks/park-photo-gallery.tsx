...
export function ParkPhotoGallery({ parkSlug }: ParkPhotoGalleryProps) {
  const [photos, setPhotos] = useState<ParkPhoto[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isAddingPhoto, setIsAddingPhoto] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);
  const [editingPhoto, setEditingPhoto] = useState<ParkPhoto | null>(null);
  ...
  const handleUpdatePhoto = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/parks/${parkSlug}/photos/${editingPhoto.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          url: url.trim(),
  ...
  const handleDeletePhoto = async (photoId: number) => {
    ...
    try {
      const response = await fetch(`/api/parks/${parkSlug}/photos/${photoId}`, {
        method: 'DELETE',
      });
  ...
  const openEditDialog = (photo: ParkPhoto) => {
  ...
  if (isLoading) {
    return null; // Don't render while loading to avoid layout shifts or empty flash
  }
      ...
      <CardContent>
        {photos.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>No photos yet</p>
            <p className="text-sm mt-1">Add photos to create your park gallery</p>
          </div>
        ) : (
          ...
          >
            ...
            >
              <img
                ...
                alt={photos[lightboxIndex].caption || 'Park photo'}
