...
export function MealPhotoEditDialog({ open, onOpenChange, meal, onSuccess }: MealPhotoEditDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [photoUrl, setPhotoUrl] = useState(meal.image_url || "");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/meals/${meal.id}/photo`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      ...
      toast.success("Meal photo URL updated successfully");
      onSuccess();
      onOpenChange(false);
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-media-background text-media-on-background border-media-outline-variant/20 font-lexend">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold tracking-tight text-media-primary">Edit Recipe Photo</DialogTitle>
          <DialogDescription className="text-media-on-surface-variant">
            Update the visual representation for &quot;{meal.name}&quot;.
          </DialogDescription>
        </DialogHeader>
