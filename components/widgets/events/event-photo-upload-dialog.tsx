...
import { type EventWithDetails } from "@/lib/db/events";
import { toast } from "sonner";
import { Upload, Link as LinkIcon, Image as ImageIcon, Loader2 } from "lucide-react";

interface EventPhotoUploadDialogProps {
  open: boolean;
...
export function EventPhotoUploadDialog({ open, onOpenChange, eventData, onSuccess }: EventPhotoUploadDialogProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/events/${eventData.event.slug}/photos/upload`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleUrlSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/events/${eventData.event.slug}/photos`, {
        method: "POST",
        headers: {
      ...
      toast.success("Event photo URL added successfully");
      onSuccess();
      onOpenChange(false);
      setPhotoUrl("");
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-media-surface text-media-primary border-media-outline-variant/20 font-lexend">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold tracking-tight">Add Event Photo</DialogTitle>
          <DialogDescription className="text-media-on-surface-variant">
            Add a new photo to the gallery for &quot;{eventData.event.title}&quot;. This will become the primary hero image.
          </DialogDescription>
        </DialogHeader>
