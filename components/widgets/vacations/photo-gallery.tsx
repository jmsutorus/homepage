...
import { VacationPhoto, Vacation } from '@/lib/types/vacations';
...
export function PhotoGallery({ vacation, photos, onUpdate }: PhotoGalleryProps) {
  const [isAddingPhoto, setIsAddingPhoto] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);
  const [editingPhoto, setEditingPhoto] = useState<VacationPhoto | null>(null);
  ...
  const handleUpdatePhoto = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/vacations/${vacation.slug}/photos/${editingPhoto.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          url: url.trim(),
  ...
  const openEditDialog = (photo: VacationPhoto) => {
      ...
      <CardContent>
        {photos.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>No photos yet</p>
            <p className="text-sm mt-1">Add photos to create your vacation gallery</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {photos.map((photo, index) => (
              ...
              >
                <img
                  src={photo.url}
                  alt={photo.caption || `Vacation photo ${index + 1}`}
          ...
          >
            ...
            >
              <img
                ...
                alt={photos[lightboxIndex].caption || 'Vacation photo'}
