...
interface VacationPosterEditDialogProps {
  open: boolean;
...
export function VacationPosterEditDialog({ open, onOpenChange, vacation, onSuccess }: VacationPosterEditDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [photoUrl, setPhotoUrl] = useState(vacation.poster || "");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/vacations/${vacation.slug}/photo`, {
        method: "POST",
        body: formData,
      });
      ...
      toast.success("Vacation poster updated successfully");
      onSuccess();
      onOpenChange(false);
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-media-surface text-media-primary border-media-outline-variant/20 font-lexend">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold tracking-tight">Edit Vacation Poster</DialogTitle>
          <DialogDescription className="text-media-on-surface-variant">
            Update the main hero image for &quot;{vacation.title}&quot;.
          </DialogDescription>
        </DialogHeader>
