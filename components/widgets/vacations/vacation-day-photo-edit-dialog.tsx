...
import { type Vacation, type ItineraryDay } from "@/lib/types/vacations";
import { toast } from "sonner";
import { Upload, Link as LinkIcon, Image as ImageIcon, Loader2, Trash2 } from "lucide-react";

interface VacationDayPhotoEditDialogProps {
  open: boolean;
...
export function VacationDayPhotoEditDialog({ open, onOpenChange, vacation, day, onSuccess }: VacationDayPhotoEditDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [photoUrl, setPhotoUrl] = useState(day.photo || "");
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(day.photo || null);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/vacations/${vacation.slug}/itinerary/${day.id}/photo/upload`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleUrlSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/vacations/${vacation.slug}/itinerary/${day.id}`, {
        method: "PATCH",
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-media-surface text-media-primary border-media-outline-variant/20 font-lexend">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold tracking-tight">Day {day.day_number} Photo</DialogTitle>
          <DialogDescription className="text-media-on-surface-variant">
            Update the photo for {day.title || `Day ${day.day_number}`}.
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="upload" className="space-y-6">
            ...
            <DialogFooter className="flex flex-col sm:flex-row gap-2">
              ...
              <Button 
                ...
                className="flex-1 bg-media-secondary hover:bg-media-secondary/90 text-white font-bold"
              >
                {isSubmitting ? (
                  <>
