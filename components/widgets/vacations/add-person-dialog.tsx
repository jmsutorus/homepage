...
export function AddPersonDialog({
  ...
  // Filter out people already added to vacation
  ...
  const handleAddPerson = async (personId: number) => {
    setIsAdding(true);
    try {
      const response = await fetch(`/api/vacations/${vacationSlug}/people`, {
        method: 'POST',
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select a person to add to this vacation.
          </DialogDescription>
        </DialogHeader>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                {availablePeople.length === 0
                  ? 'All people have been added to this vacation!'
