...
import { VacationPerson } from '@/lib/types/vacations';
import { showCreationSuccess, showCreationError } from '@/lib/success-toasts';
import { AddPersonDialog } from './add-person-dialog';

interface VacationPeopleSectionProps {
  vacationSlug: string;
...
export function VacationPeopleSection({ vacationSlug, people, onUpdate }: VacationPeopleSectionProps) {
  ...
  const handleRemove = async (personAssociationId: number, personName: string) => {
    ...
    try {
      const response = await fetch(`/api/vacations/${vacationSlug}/people/${personAssociationId}`, {
        method: 'DELETE',
      });
        ...
        <CardContent>
          {people.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">
              No travel companions added yet. Click &quot;Add Person&quot; to get started!
            </p>
          ) : (
