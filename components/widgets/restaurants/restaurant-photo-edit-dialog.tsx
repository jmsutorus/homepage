...
interface RestaurantPhotoEditDialogProps {
  open: boolean;
...
export function RestaurantPhotoEditDialog({ open, onOpenChange, restaurant, onSuccess }: RestaurantPhotoEditDialogProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [photoUrl, setPhotoUrl] = useState(restaurant.poster || "");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/restaurants/${restaurant.slug}/photo`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-media-surface text-media-primary border-media-outline-variant/20 font-lexend">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold tracking-tight">Edit Restaurant Photo</DialogTitle>
          <DialogDescription className="text-media-on-surface-variant">
            Update the main image for &quot;{restaurant.name}&quot;.
          </DialogDescription>
        </DialogHeader>
