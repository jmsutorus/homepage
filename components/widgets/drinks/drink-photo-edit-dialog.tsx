...
export function DrinkPhotoEditDialog({ open, onOpenChange, drink, onSuccess }: DrinkPhotoEditDialogProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success("Drink photo updated successfully");
      onSuccess();
      onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-media-surface text-media-primary border-media-outline-variant/20 font-lexend">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold tracking-tight">Edit Drink Photo</DialogTitle>
          <DialogDescription className="text-media-on-surface-variant">
            Update the main image for &quot;{drink.name}&quot;.
          </DialogDescription>
        </DialogHeader>
