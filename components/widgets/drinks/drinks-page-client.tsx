        ...
        {drinks.length === 0 ? (
          <div className="flex min-h-[60vh] items-center justify-center w-full py-12">
            <div className="flex flex-col items-center text-center px-4 max-w-md">
              <div className="w-24 h-24 rounded-full bg-[var(--color-media-secondary)]/10 flex items-center justify-center text-[var(--color-media-secondary)] mb-6 shadow-inner">
                <span className="material-symbols-outlined text-5xl">wine_bar</span>
              </div>
              <h2 className="text-3xl font-black text-[var(--color-media-primary)] mb-3 tracking-tight">Your Collection is Empty</h2>
              <p className="text-[var(--color-media-on-surface-variant)] font-medium mb-8">
                Start documenting your favorite pours and tastings. Create a curated record of your liquid journey.
              </p>
              <button 
                onClick={() => { haptic.trigger("light"); setShowForm(true); }}
                className="cursor-pointer bg-[var(--color-media-secondary)] text-white px-8 py-4 rounded-xl font-bold flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-lg shadow-[var(--color-media-secondary)]/20 border-none"
              >
